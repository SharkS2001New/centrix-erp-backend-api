-- ================================================================
-- Centrix ERP DATABASE SCHEMA  v3.0
-- ================================================================
...
CREATE TABLE users (
    ...
    mobile_order_scope ENUM('route_only','normal_only','both') NULL DEFAULT 'both',
...
CREATE TABLE stock_take_sessions (
    ...
    filter_subcategory_id INT NULL,
    filter_supplier_id INT NULL,
...
CREATE TABLE employee_cash_advances (
    ...
    status             ENUM('pending','open','repaid','cancelled') NOT NULL DEFAULT 'pending',
    repayment_mode     ENUM('full_next_cycle','fixed_per_cycle') NOT NULL DEFAULT 'full_next_cycle',
...
GROUP BY it.branch_id, it.product_code, p.product_name, p.unit_id, cs.shop_quantity, cs.store_quantity;
...
DROP VIEW IF EXISTS v_statutory_deductions;
...
DROP VIEW IF EXISTS v_bank_transfer_report;
...
DROP VIEW IF EXISTS v_nssf_remittance_report;
...
DROP VIEW IF EXISTS v_other_deductions_by_period;
...
DROP VIEW IF EXISTS v_headcount_report;
...
DROP VIEW IF EXISTS v_contract_expiry;
...
DROP VIEW IF EXISTS v_staff_turnover;
...
DROP VIEW IF EXISTS v_hr_dashboard_kpi;
...
-- END SCHEMA v3.1 — Tables: 56 | Triggers: 2 | Views: 36
