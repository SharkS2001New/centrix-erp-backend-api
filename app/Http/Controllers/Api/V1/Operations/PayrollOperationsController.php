...
class PayrollOperationsController extends Controller
{
    ...
    public function processRun(Request $request, string $runId)
    {
        ...
        return DB::transaction(function () use ($request, $run, $payload, $autoCalculate, $closeCycle, $settlementOptions, $orgId) {
            ...
            foreach ($payload['lines'] as $lineInput) {
                ...
                $withNssf = (bool) ($employee->pays_nssf ?? true);
                $withHousing = (bool) ($employee->pays_housing_levy ?? true);
                $withPaye = (bool) ($employee->pays_paye ?? true);

                if ($autoCalculate || ! isset($lineInput['paye'])) {
                    $calc = $this->calculator->calculateMonthly(
                        $statutoryGross,
                        $other,
                        0,
                        $orgId,
    ...
    protected function manualLine(
        array $lineInput,
        float $gross,
        float $other,
        bool $withShif = true,
        bool $withNssf = true,
        bool $withHousing = true,
        bool $withPaye = true,
    ): array {
        $nssf = $withNssf ? (float) ($lineInput['nssf'] ?? 0) : 0.0;
        $shif = $withShif ? (float) ($lineInput['shif'] ?? 0) : 0.0;
        $housing = $withHousing ? (float) ($lineInput['housing_levy'] ?? 0) : 0.0;
        $paye = $withPaye ? (float) ($lineInput['paye'] ?? 0) : 0.0;
