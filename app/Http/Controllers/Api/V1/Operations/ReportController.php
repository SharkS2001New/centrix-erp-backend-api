...
class ReportController extends Controller
{
    ...
    public function salesByUser(Request $request)
    {
        ...
        return response()->json($this->reportFromViewOverall(
            ...
            ['order_count', 'collected_order_count', 'unpaid_order_count', 'gross_sales', 'total_vat', 'net_sales', 'amount_collected', 'fully_paid_sales', 'unpaid_sales', 'outstanding_balance', 'cogs', 'gross_profit'],
            [],
    ...
    public function paymentsBreakdown(Request $request)
    {
        ...
        return response()->json(
            app(\App\Services\Reports\PaymentsBreakdownService::class)->build(
                $request,
                (int) $orgId,
                $branchId,
            ),
    ...
    /** Hotel POS end-of-day dashboard — same payload shape as retail eod-report. */
    public function hospitalityEodReport(Request $request)
    {
        $access = app(UserAccessService::class);
        $orgId = $access->organizationId($request->user(), $request);
        abort_unless($orgId, 403, 'Your account is not linked to an organization.');
        ...
        return response()->json(
            app(\App\Services\Reports\HospitalityEodReportService::class)->build(
                $request,
                (int) $orgId,
                $branchId,
            ),
