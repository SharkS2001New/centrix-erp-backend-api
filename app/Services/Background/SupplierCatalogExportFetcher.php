...
class SupplierCatalogExportFetcher
{
    public function __construct(
        protected SupplierController $suppliers,
    ) {}
    ...
    public function eachPage(
        ...
        do {
            ...
            $request = Request::create('/api/v1/suppliers', 'GET', $query);
            $request->setUserResolver(static fn () => $user);

            $response = $this->suppliers->index($request);
            if ($response->getStatusCode() >= 400) {
                throw new RuntimeException('Could not load suppliers for export (HTTP '.$response->getStatusCode().').');
            }

            $payload = json_decode($response->getContent(), true);
            if (! is_array($payload)) {
                throw new RuntimeException('Could not load suppliers for export (invalid response).');
            }
            ...
            if ($onProgress !== null && $lastPage > 0) {
                ...
                $onProgress($progress, 'Loading suppliers…');
            }
            $page++;
        } while ($page <= $lastPage);
