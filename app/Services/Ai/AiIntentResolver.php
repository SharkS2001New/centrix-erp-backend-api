...
class AiIntentResolver
{
    ...
    protected function matchesOpenLpo(string $text): bool
    {
        ...
        // Status / goods-receiving list questions belong to list_lpos tool, not bare /lpo navigate.
        if (preg_match(
            '/\b(awaiting|partial(ly)?|goods\s+receiv|to\s+receive|grn|unreceived|not\s+yet\s+received|still\s+to\s+receive)\b/i',
            $text,
        )) {
            return false;
        }
