...
class XeroExportDriver implements ExternalAccountingExportDriver
{
    public function exportJournal(AccountingExportQueue $item): string
    {
        ...
        if (! $connection) {
            throw new \RuntimeException('Xero is not connected for this organization.');
        }

        $mappedLines = $this->applyMappings((int) $item->organization_id, $item->lines ?? []);
        if ($mappedLines === []) {
            throw new \RuntimeException('No mappable journal lines for export. Map chart of accounts to Xero first.');
        }

        if (! config('xero.client_id') || ! config('xero.client_secret')) {
            Log::info('Xero export stub', [
                'organization_id' => $item->organization_id,
                'entry_number' => $item->entry_number,
                'lines' => $mappedLines,
            ]);

            return 'XERO-STUB-'.Str::upper(Str::random(8));
        }

        throw new \RuntimeException('Xero live export is not implemented yet. Configure stub mode or complete the API driver.');
    }
