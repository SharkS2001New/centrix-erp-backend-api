...
Route::prefix('v1')->group(function () {
    ...
    Route::middleware(['auth:sanctum', 'erp.tenant', 'erp.session_idle', 'erp.password_expiry', 'throttle:api'])->group(function () {
        ...
        Route::post('admin/slow-queries/advise', [\App\Http\Controllers\Api\V1\PlatformSlowQueryController::class, 'advise'])
            ->middleware(['erp.super_admin']);
        Route::post('admin/slow-queries/advise-url', [\App\Http\Controllers\Api\V1\PlatformSlowQueryController::class, 'adviseUrl'])
            ->middleware(['erp.super_admin']);
        ...
        Route::post('admin/operational-prune', [\App\Http\Controllers\Api\V1\PlatformOperationalPruneController::class, 'run'])
            ->middleware(['erp.super_admin']);
        Route::post('admin/operational-prune/stream', [\App\Http\Controllers\Api\V1\PlatformOperationalPruneController::class, 'runStream'])
            ->middleware(['erp.super_admin']);
        ...
        Route::apiResource('branches', BranchController::class)
            ->only(['index', 'show'])
            ->middleware([
                'erp.module:admin,hr_payroll,sales,inventory,accounting,distribution,customers_suppliers,sales.pos',
                'erp.permission:admin.view|hr.view|hr.manage|fulfillment.manage|fulfillment.view|customers.view|customers.manage|sales.view|inventory.view|accounting.view|reports.view|catalogue.view|catalogue.products.view|pos.checkout.create|pos.terminal.view|pos.till',
            ]);
        ...
        Route::middleware(['erp.module:inventory'])->group(function () {
            ...
            Route::get('products/catalog-summary', [ProductController::class, 'catalogSummary'])
                ->middleware(['erp.permission:catalogue.view|catalogue.products.view|inventory.view|purchasing.view|pos.checkout.create|pos.terminal.view']);
            ...
            Route::apiResource('products', ProductController::class)
                ->middlewareFor(['index', 'show'], ['erp.permission:catalogue.view|catalogue.products.view|inventory.view|purchasing.view|pos.checkout.create|pos.terminal.view'])
                ->middlewareFor(['store', 'update', 'destroy'], ['erp.permission:products.manage']);
        ...
        Route::middleware(['erp.module:distribution'])->group(function () {
            ...
            Route::post('dispatch-trips/{trip}/start', [DispatchTripController::class, 'start'])
                ->middleware('erp.permission:fulfillment.manage');
            Route::post('dispatch-trips/{trip}/confirm-deliveries', [DispatchTripController::class, 'confirmDeliveries'])
                ->middleware('erp.permission:fulfillment.manage');
