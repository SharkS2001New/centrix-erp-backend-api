...
class StockTransferApprovalTest extends TestCase
{
    ...
    public function test_donations_purpose_transfer_appears_in_stock_transfers_report(): void
    {
        $admin = User::where('username', 'admin')->firstOrFail();
        $product = Product::query()->firstOrFail();
        $this->seedStoreStock($admin, $product->product_code, 20);
        ...
        $this->assertDatabaseHas('stock_movement_history', [
            ...
            'branch_id' => $admin->branch_id,
            'from_location' => 'store',
            'to_location' => 'donations',
            'quantity_moved' => 3,
            'moved_by' => $admin->id,
            'notes' => 'Community drive',
        ]);

        $row = DB::table('v_stock_transfers')
            ->where('product_code', $product->product_code)
            ->where('branch_id', $admin->branch_id)
            ->where('to_location', 'donations')
            ->whereDate('transfer_date', now()->toDateString())
            ->first();

        $this->assertNotNull($row, 'Donations transfer should appear in v_stock_transfers');
        $this->assertEqualsWithDelta(3.0, (float) ($row->total_moved ?? 0), 0.001);
        $this->assertSame('Community drive', $row->notes ?? null);
    }
}
